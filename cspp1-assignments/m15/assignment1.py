...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        for shift_range in range(27):
            message = PlaintextMessage(self.message_text, shift_range)
            decrypted = message.get_message_text_encrypted()
            valid_words = 0
            for word in decrypted.split(' '):
                if is_word(self.valid_words, word):
                    valid_words += 1
            if self.max_valid_words < valid_words:
                self.max_valid_words = valid_words
                self.decrypted_message = (26 - shift_range, decrypted)
